...
namespace 纸娃娃系统
{
    ...
    public partial class SetHead_Female : Form
    {
        #region 全局变量
        int Fag = 0;
        Head hear = new Head();
        ...
        public SetHead_Female(zwwParentWin _from)
        {
            InitializeComponent();
        ...
        private void Head_attribute_Face_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Head_attribute_Eyebrow_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Head_attribute_Eye_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Head_attribute_Nose_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Head_attribute_Mouth_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Head_attribute_Hair_Front_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Head_attribute_Hair_Back_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Head_attribute_Ear_CheckedChanged(object sender, EventArgs e)
        {
        ...
        #region 改变颜色
        //眼睛颜色
        private void trackBar_hue_Eye_Scroll(object sender, EventArgs e)
        {
            hear.hue_Eye = this.trackBar_hue_Eye.Value;
            RefreshHeadPhoto();
        }
        private void trackBar_saturation_Eye_Scroll(object sender, EventArgs e)
        {
            hear.saturation_Eye = this.trackBar_saturation_Eye.Value * .01;
            RefreshHeadPhoto();
        }
        private void trackBar_lightness_Eye_Scroll(object sender, EventArgs e)
        {
        ...
        private void Initial_texture_Window()
        {
            ...
            try
            {
                string FrontHairpath = @".\texture\Female\Hair_Front";
                string Facepath = @".\texture\Female\Face_shape";
                string Earpath = @".\texture\Female\Ear";
                string Eyebrowpath = @".\texture\Female\Eyebrow";
                string Eyepath = @".\texture\Female\Eyes";
                string BackHairDoublepath = @".\texture\Female\Hair_Back\double";
                string BackHaironepath = @".\texture\Female\Hair_Back\one";
                string Mouthpath = @".\texture\Female\Mouth";
                string Nosepath = @".\texture\Female\Nose";
                ...
                for (int i = 0; i < EyeTheFolder.GetFiles().Length; i++)
                {
                    ...
                    {
                        //只显示部分就行了(不显示B)
                        if (EyeTheFolder.GetFiles()[i].Name.Contains("A"))
                        {
                            //string result = System.Text.RegularExpressions.Regex.Replace(EyeTheFolder.GetFiles()[i].Name, @"[^0-9]+", "");
                            //TempEyeFag = int.Parse(result);
                ...
                #region 给hear赋初始值
                //给hear赋初值;
                hear.Hair_Front_path = @".\texture\Female\Hair_Front\" + FrontHairNames[0];
                hear.Face_path = @".\texture\Female\Face_shape\" + FaceNames[0];
                hear.Ear_path = @".\texture\Female\Ear\" + EarNames[0];
                hear.Eyebrow_path = @".\texture\Female\Eyebrow\" + EyebrowNames[0];
                hear.Eye_A_path = @".\texture\Female\Eyes\" + EyeANames[0];
                hear.Eye_B_path = @".\texture\Female\Eyes\" + EyeBNames[0];
                hear.is_HairFrontDouble = false;
                hear.Hair_Back_A_path = @".\texture\Female\Hair_Back\one\" + BackHairANames[BackHairFag];
                hear.Hair_Back_B_path = @".\texture\Female\Hair_Back\one\" + BackHairANames[BackHairFag];
                hear.Mouth_path = @".\texture\Female\Mouth\" + MouthNames[0];
                hear.Nose_path = @".\texture\Female\Nose\" + NoseNames[0];
                hear.hue_Eye = this.trackBar_hue_Eye.Value;
                hear.saturation_Eye = this.trackBar_saturation_Eye.Value * .01;
                hear.lightness_Eye = this.trackBar_lightness_Eye.Value * .01;
                hear.hue = this.trackBar_hue.Value;
        ...
        //列表选择图片
        private void ListView_Hair_Front_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void RefreshSelectiem(int _seleIndex)
        {
            ...
            {
                case 0:
                    hear.Face_path = @".\texture\Female\Face_shape\" + FaceNames[_seleIndex];
                    break;
                case 1:
                    hear.Eyebrow_path = @".\texture\Female\Eyebrow\" + EyebrowNames[_seleIndex];
                    break;
                case 2:
                    hear.Eye_A_path = @".\texture\Female\Eyes\" + EyeANames[_seleIndex];
                    hear.Eye_B_path = @".\texture\Female\Eyes\" + EyeBNames[_seleIndex];
                    break;
                case 3:
                    hear.Nose_path = @".\texture\Female\Nose\" + NoseNames[_seleIndex];
                    break;
                case 4:
                    hear.Mouth_path = @".\texture\Female\Mouth\" + MouthNames[_seleIndex];
                    break;
                case 5:
                    hear.Hair_Front_path = @".\texture\Female\Hair_Front\" + FrontHairNames[_seleIndex];
                    break;
                case 6:
                    if (_seleIndex >= BackHairFag)
                    {
                        hear.is_HairFrontDouble = false;
                        hear.Hair_Back_B_path = @".\texture\Female\Hair_Back\one\" + BackHairANames[_seleIndex];
                    }
                    else
                    {
                        hear.is_HairFrontDouble = true;
                        hear.Hair_Back_A_path = @".\texture\Female\Hair_Back\double\" + BackHairANames[_seleIndex];
                        hear.Hair_Back_B_path = @".\texture\Female\Hair_Back\double\" + BackHairBNames[_seleIndex];
                    }
                    break;
                case 7:
                    hear.Ear_path = @".\texture\Female\Ear\" + EarNames[_seleIndex];
                    break;
                default:
                    break;
            }
        ...
        private void RefreshHeadPhoto()
        {
            ...
            Bitmap NeckBmp = new Bitmap(@".\texture\Female\Neck.png");
        ...
        //正片叠底（该代码不调用，教学用）
        private Bitmap PositiveOverlap(Bitmap srcBitmap, Color _color)
        {
            //遍历像素的方法
            //if (srcBitmap != null)
            //{
            ...
            //        {
            //            curColor = srcBitmap.GetPixel(i, j);
            //            if (curColor.A < 1)
            //            {
            //                continue;
            //            }
            //            byte temp = (byte)(curColor.R * .299 + curColor.G * .587 + curColor.B * .114);
            //            srcBitmap.SetPixel(i, j, Color.FromArgb(temp, temp, temp));
            //        }
            //    }
            //}
            ...
            if (srcBitmap != null)
            {
                ...
                for (int i = 0; i < rgbvalues.Length; i += 4)
                {
                    ...
                    //a = (colortemp * _color.R) / 255; b = (colortemp * _color.G) / 255; c = (colortemp * _color.B) / 255;
                    a = (rgbvalues[i + 2] * _color.R) / 255; b = (rgbvalues[i + 1] * _color.G) / 255; c = (rgbvalues[i] * _color.B) / 255;
                    rgbvalues[i] = (byte)a;
        ...
        private void button_Save_Click(object sender, EventArgs e)
        {
            ...
            saveFile.Filter = "头部文件|*.fmheadfile*";
            saveFile.RestoreDirectory = true;
            saveFile.FilterIndex = 1;
            saveFile.FileName = "temp.fmheadfile";
        ...
        private void Button_load_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog.Filter = "头部文件|*.fmheadfile*";
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                BinaryFormatter bf = new BinaryFormatter();
                hear = bf.Deserialize(file) as Head;
                //读取之后再刷新一下
                RefreshHeadPhoto();
                file.Close();
            }
